...
plot_list_n <- 
  if(plottype == "bands"){
    map2(site_char_list_n$Site, site_char_list_n$Characteristic, 
      ...
      pmap(list(site_char_list_n$Site, site_char_list_n$Characteristic, boxplot_legend_n),
...
for(i in seq_along(site_list)){
  ...
  for (i in 1:(num_plots_n - 1)){
  subplots_n$x$data[[i]]$showlegend <- FALSE #site (line/markers)
  subplots_n$x$data[[length(subplots_n$x$data)]]$showlegend <- FALSE #threshold line
  #subplots_n$x$data[[length(subplots_n$x$data)]]$name <- "WQ Threshold" #set name
  }
  
  #++++++++++++++++++ ENDED HERE +++++++++++++++++++++
  # The legend for the boxplots is still printing duplicates
  #+++++++++++++++++++++++++++++++++++++++++++++++++++
