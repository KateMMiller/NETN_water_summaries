...
params:
  ...
  plottype: boxplot # plot bands or boxplot for nutrients
---
...
char_df_lake_n <- wdata_curr |> 
  filter(month_num >= 5) |>
  select(Site, Characteristic, SiteName, SiteType, Category) |> 
  filter(Characteristic %in% char_list_lake_n) |> 
  filter(SiteType == "Lake")|> unique() 
...
plot_list_lake_n <- 
    if(plottype == "bands"){
      ...
      pmap(list(char_df_lake_n$Site, char_df_lake_n$Characteristic, boxplot_legend_lake_n),
          ~waterboxplot(sitecode = ..1, charname = ..2, legend = ..3)) |> 
           set_names(char_df_lake_n$Site)
}
...
char_list_stream_p <- sort(unique(
  wdata_curr$Characteristic[wdata_curr$Category == 'physical' & 
                              ...
                              wdata_curr$Characteristic != "Discharge_cfs"]))
...
plot_list_stream_n <- 
    if(plottype == "bands"){
      ...
      pmap(list(char_df_stream_n$Site, char_df_stream_n$Characteristic, boxplot_legend_stream_n),
          ~waterboxplot(sitecode = ..1, charname = ..2, legend = ..3)) |> 
           set_names(char_df_stream_n$Site)
      }
